...
def train_classifier_RF_Validation(X: numpy.ndarray, y: numpy.ndarray, type_labels=None, stratify_labels=None):
   ...
   type_rates_path = os.path.join(table_path, "RF_validation_types.csv")
            ...
            for depth in maxium_depths:
               ...
               if type_labels is not None:
                  for malicious_type, detected, total, rate in compute_type_detection_rates(y, y_pred, type_labels):
                     type_rows.append([f'estimators = {estimator}; max_depth={depth}', malicious_type, detected, total, rate])
